...
import json,sys,os,time,secrets,hashlib,hmac
...
INDEX=Path(os.environ.get("ESCL_INDEX","index.html"))
RATE={}
DEV_OUTBOX=[]
...
class App(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if p.startswith("/api/") and p not in ("/api/auth/register","/api/auth/login") and not self.require_user():return
            if p.startswith("/api/") and p not in ("/api/auth/register","/api/auth/login","/api/auth/forgot-password","/api/auth/reset-password","/api/auth/logout","/api/auth/logout-all","/api/auth/resend-verification","/api/legal/accept","/api/admin/moderate") and not p.startswith("/api/commish/"):
                if not self.require_verified():return
            if p=="/api/driver/create":
                u=self.require_verified()
                if not u:return
                return self.out(svc.create_driver(b.get("name"),b.get("number"),b.get("attributes",{}),b.get("hometown",""),b.get("team_id"),u["id"]))
...
if __name__=="__main__":
    ...
    print(f"ESCL v6.0 on http://{HOST}:{PORT}")
